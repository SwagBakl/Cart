...
<div class="col-3" id="add_form shadow-lg">
    <h3 class="display">Добавление характеристик домашнего кинотеатра<br> <b>${model}</b></h3>
    <form method="post" action="/ht/${id}/characts">
...
<div class="col-8 card-col-sum">
    <table class="table shadow-lg">
        ...
        <tr>
            ...
            <#list htChars as char>
            <tr>
                <td><b>${char.name}</b></td>
                <td><b>${char.value}</b></td>
                <#if isAdmin>
                <td><a href="/ht/${char.id}" class="btn btn-warning shadow-lg">
                    <span class="d-inline-flex">Редактировать
                    <i class="material-icons ml-2">
                        edit
                    </i></span>
                </a></td>
                <td><a href="/ht/characts/${char.id}/delete" class="btn btn-warning shadow-lg">
